...
public class BoardBuilder3D : MonoBehaviour {
    ...
    private void CreateFloor(Cell[,] grid, int gridWidth, int gridHeight) {
        var passablePositions = new List<Vector2Int>();
        for(int x = 0; x < gridWidth; x++)
            for(int y = 0; y < gridHeight; y++)
                if(grid[x,y].isPassable && !grid[x,y].isHole)
                    passablePositions.Add(new Vector2Int(x, y));
        
        if(passablePositions.Count > 0) {
            var floorMesh = GenerateQuadMesh(passablePositions, 0f);
    ...
    private void CreateWalls(Cell[,] grid, int gridWidth, int gridHeight) {
        ...
        for(int x = 0; x < gridWidth; x++) {
            for(int y = 0; y < gridHeight; y++) {
                if(grid[x,y].tile?.CompareTag("Wall") == true && !visited[x,y]) {
    ...
    private void CreateHoles(Cell[,] grid, int gridWidth, int gridHeight) {
        ...
        for(int x = 0; x < gridWidth; x++) {
            for(int y = 0; y < gridHeight; y++) {
                if(grid[x,y].isHole && !visited[x,y]) {
